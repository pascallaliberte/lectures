...
export async function renderExtraPage(extras, extra, dist, views, ensureUniqueLecturesReducer, formatReading, getAllLecturesFromAllMesses) {
  ...
  console.log(date_string, JSON.stringify(details))
  
  var json = await fetch('https://api.aelf.org/v1/messes/' + date_string + '/canada')
  .then(function(r) {
    return r.json();
  });
  ...
  ejs.renderFile(views + 'extras.ejs', { 
    ...
    lectures: lectures.reduce(ensureUniqueLecturesReducer, []).map(function(lecture) {
